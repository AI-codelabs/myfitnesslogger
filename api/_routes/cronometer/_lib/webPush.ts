// Cronometer web-scraper for target pushing ONLY (ported from
// supabase/functions/cronometer/webPush.ts). Read paths (diary, macros) all
// use the official Pro API in proClient.ts.
//
// Design goals (unchanged from the Deno version):
...
//   3. AES-GCM encryption for credentials + cookies at rest (see crypto.ts).
//   4. Every network call logged into cronometer_api_logs for auditability.
...
export async function loadCurrentTargets(
  ...
  const { rows } = await sql.query<{ details: Record<string, unknown> | null }>(
    `SELECT details FROM public.nutrition_plans WHERE client_id = $1 LIMIT 1`,
    [clientId],
  );
  const det = rows[0]?.details ?? null;
  if (!det) return null;
  return {
