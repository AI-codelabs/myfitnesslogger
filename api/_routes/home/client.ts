...
export default endpoint({ method: "GET", schema }, async ({ sql, user, input }) => {
  ...
  // Sequential on one transaction client — node-pg forbids concurrent queries.
  const profileRes = await sql.query<{ first_name: string | null; display_name: string | null }>(
    `SELECT first_name, display_name FROM public.profiles WHERE user_id = $1 LIMIT 1`,
    [uid],
  );
  const checkinRes = await sql.query<{ submitted_at: string | null }>(
    `SELECT submitted_at FROM public.weekly_checkins
      WHERE client_id = $1 AND week_start = $2::date LIMIT 1`,
    [uid, weekStart],
  );
  const sessionsRes = await sql.query<{ id: string; completed_at: string | null }>(
    `SELECT id, completed_at FROM public.workout_sessions
      WHERE client_id = $1 AND scheduled_date = $2::date`,
    [uid, today],
  );
  const weeklyRes = await sql.query(
  ...
  const startMsgRes = await sql.query(
  ...
  const planRes = await sql.query<{ details: unknown }>(
    `SELECT details FROM public.nutrition_plans
      WHERE client_id = $1
      ORDER BY created_at DESC LIMIT 1`,
    [uid],
  );
  const nutritionLogRes = await sql.query<{
  ...
  const complianceRes = await sql.query<{ log_date: string }>(
  ...
  const goalRes = await sql.query(
    `SELECT * FROM public.client_goals
      WHERE client_id = $1 AND is_active = true
      ORDER BY created_at DESC LIMIT 1`,
    [uid],
  );
  const weightRes = await sql.query<{ weight_kg: number }>(
    `SELECT weight_kg FROM public.weight_logs
      WHERE client_id = $1
      ORDER BY logged_on DESC LIMIT 1`,
    [uid],
  );
  const progressionRes = await sql.query<{
    week_start: string;
